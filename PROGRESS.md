...
| 26 | 画质：后处理与抗锯齿 | ✅ | code/ch26-quality（src/main.rs 即 Listing 26-13《定妆照》画质开关面板（1-5 磨边/B 辉光/F 景深/M 运动模糊/V 老镜头/T 冲印 + 中文状态牌）+ examples/listing-26-01～11；报错环节双份：no-compile/listing-26-12 把 Msaa 当资源插的 E0277（编译器「加 derive」歧路提示）+ 26-11 TAA×MSAA 每帧 warn 刷屏（约 60 条/秒实测）；哑巴坑两位：拔 Hdr 辉光静默熄灭（extract With<Hdr>）、AutoExposure 忘挂插件零反应；实测翻车修正入册：夜景欠曝正解=Exposure::INDOOR（22.2 回访）、**默认 45° 广角下景深隐形**（弥散圆∝焦距²，fov 收 0.35=53mm 长焦才有戏——正文警示）、AutoExposure speed_brighten/darken 主语是场景不是画面（审阅抓出，改成显式命名警示）、运行时拆装 ContactShadows 触发 wgpu 校验崩溃（疑引擎 bug，绕开记档）；TAA×接触阴影噪点熔平实验清 22.7 的账；资产仅复用 ch16 字体（make_ch26_assets.py））；book/src/ch26-00～13 共 14 节（38.9k 字符）；插图 images/ch26（Figure 26-1～25，含 3 张手绘 SVG、1 张频闪对比动图，scripts/make_ch26_figures.py 一键重建）；26.13 内嵌 WASM demo（scripts/build_ch26_wasm.py 一键重建，宿主页入库、产物 26.1MB gitignore；**两条 WebGL2 红线实测撞出**：MSAA×prepass 纹理 glow 层 panic、TAA 着色器 naga 译不动 GLSL ES→AppExit，均以 cfg!(wasm32) 互斥/封存修复并双端回归，红线与三条驱动坑归档 wasm-ops.md；全键路真 Chrome 实测）；工单 workorders/ch26.md（试行标尺第四章：逐参数 60+ 字段/微实验 14 组入文，38.9k/标尺 45k，审阅 A4/B4/C12 全处置） |
| 27 | Gizmos、诊断与开发工具 | ⬜ | |
| 28 | UI 基础：Node 与布局 | ⬜ | |

...
| 25 | Picking 与相机控制 | ✅ | code/ch25-picking-camera-control（src/main.rs 即 Listing 25-6 点选拖拽舞台 + examples/listing-25-01～05，其中 25-02 在 no-compile/ 下为编译失败示例（`Pointer<Drag>::delta` 是屏幕像素 `Vec2`，不能直接加到 `Transform::translation` 的 `Vec3`）；开 bevy `free_camera` feature，mesh picking 用 `MeshPickingSettings::require_markers` + `MeshPickingCamera` + `Pickable` 显式 opt-in；中文 HUD 字体复用 ch16，经 scripts/make_ch25_assets.py 一键复制）；book/src/ch25-00～06；插图 images/ch25（Figure 25-1～6，含 3 张手绘 SVG：picking 管线、Pickable 遮挡层、FreeCamera 控制图；3 张截图由 scripts/make_ch25_figures.py 一键重建）；**WASM 网页 demo**：src/main.rs（`web_window` canvas `#bevy-ch25` + 左键点选/拖拽物体 + 右键 FreeCamera），scripts/build_ch25_wasm.py 编出 ch25_stage（~20.7 MB）入 book/src/demos/ch25，懒加载模板内嵌，已在浏览器真 GPU 实跑验证（渲染/中文字体/左键点选/左键拖拽） |
| 26 | 画质：后处理与抗锯齿 | ✅ | `code/ch26-post-processing-aa`：画质开关面板，覆盖 HDR/Tonemapping、Bloom、DepthOfField、MotionBlur、MSAA/FXAA/SMAA/TAA、CAS、ChromaticAberration；正文 `book/src/ch26-00`～`ch26-06`；插图 `book/src/images/ch26/`（Figure 26-1～4），PNG 由 `scripts/make_ch26_figures.py` 一键重建，字体由 `scripts/make_ch26_assets.py` 复制；未做 wasm demo：Bevy 0.18.1 源码标注 Bloom 当前不兼容 WebGL2，留待 WebGPU/实验功能章节统一处理 |
| 27 | Gizmos、诊断与开发工具 | ⬜ | |
| 28 | UI 基础：Node 与布局 | ⬜ | |

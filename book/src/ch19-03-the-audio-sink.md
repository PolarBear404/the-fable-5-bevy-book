...
场记：第 1 帧——曲谱到货。
场记：第 2 帧——AudioSink 上岗。音量 Linear(1.0)，速度 ×1，暂停 false，进度 0.02 秒。
```

台账上有两个间隔。**`Startup` 里 spawn，第 1 帧查不到 sink**——哪怕曲谱当帧就到了货（小文件加载快，第 14 章的库房广播第 1 帧就报了 `LoadedWithDependencies`）。因为引擎的开播系统住在 `PostUpdate`：它看到“有 `AudioPlayer`、资产已到货、还没有 sink”的实体，才创建声音通路、用 `Commands` 把 `AudioSink` 插上去——命令要到同步点才落账（第 6 章的规矩），所以**最早也得第 2 帧才查得到**。要是换一个大文件或者慢磁盘，“到货”本身还会再晚几帧，sink 跟着顺延。
...
![三联截图：开戏时戏台钟与曲子进度同为 2.9 秒并排走；按空格中场后阿燕定格、戏台钟停在 3.1 秒、曲子进度却涨到 6.4 秒仍标着奏；再按 P 后曲子进度停在 6.5 秒标停](images/ch19/fig-19-03-two-pauses.png)

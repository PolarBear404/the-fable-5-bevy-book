...
![左右对照（局部放大到台中绣球）：左幅球面爬满细密的斜向暗纹、脏成一团，是 shadow acne；右幅同一只球光洁润泽，受光面干干净净](images/ch22/fig-22-04-shadow-acne.png)

<span class="caption">Figure 22-4：放大看台中绣球——左边 bias 归零，受光面爬满自阴影噪点（shadow acne）；右边默认 bias，光洁如初</span>
...
![示意图：左半幅 bias=0，一道受光斜面与阴影贴图量化后的「阶梯」深度交错，斜面落到阶梯下方的几段被红色标出——比记录更远、误判为自阴影（acne）；右半幅加上 bias，比对深度被整体外推、阶梯沉到斜面之下，斜面整段更近、不再误伤，全亮](images/ch22/fig-22-05-shadow-bias.svg)

<span class="caption">Figure 22-5：bias 治 acne 的道理——量化的贴图深度像一道阶梯，与斜面交错处会把自己误判成自阴影；bias 把比对深度外推一点，斜面整段就不再误伤</span>

默认值（0.02 / 1.8）是 Bevy 给大多数场景调好的，Listing 22-2 用的就是它们，所以那张图干干净净。把它们归零就退化成 Figure 22-4 左边的样子。反过来，bias 也不能贪大：推过头，影子会从投影物脚下「脱开」一道缝，像物体浮在自己的影子上——这毛病叫 **peter-panning（彼得潘效应，影子跟人脱了钩）**。bias 的活就是在 acne 与 peter-panning 之间找那个甜区，默认值通常已经够好，真要调，小步挪。
...
![示意图：相机在左，视锥向右展开，按距离切成近窄远宽的几段、各染一色；下方画两张同样分辨率的贴图——近段那张覆盖小范围、格子细，远段那张覆盖大范围、格子粗，虚线标出对应关系](images/ch22/fig-22-06-cascades.svg)

<span class="caption">Figure 22-6：级联把视锥按距离切段，每段各用一张同分辨率的贴图——近段覆盖小范围所以精细、远段覆盖大范围所以粗，精度因此集中到镜头跟前</span>

...
- **驿站的慢递**——三息定场、中桩报“中”：到点办一次的活不必养表，把命令排给延迟投递，顺便审清楚驿站听哪只钟；
...
这些类型住在 `bevy_time` 这个 crate 里。常用的（`Time`、`Fixed`、`Real`、`Virtual`、`Timer`、`TimerMode`，以及延迟命令的入口 `DelayedCommandsExt`）都从 `bevy::prelude` 来；`Stopwatch` 与定时器系的运行条件（`on_timer`、`on_real_timer`、`paused`）要从 `bevy::time` 显式引入。调度侧的 `FixedUpdate`、`RunFixedMainLoop`、`RunFixedMainLoopSystems` 在 `bevy_app`，也都进了 prelude。

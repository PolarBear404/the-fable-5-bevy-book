...
```text
—— 第 1 帧 ——
  账房：镇仓的账动了！现存 50 袋（added=true，last_changed=2，窗口 (1036800005, 7]）
—— 第 2 帧 ——
  小工掸灰，把账目原样抄了一遍（解引用了 &mut，值没变）。
  账房：镇仓的账动了！现存 50 袋（added=false，last_changed=28，窗口 (7, 29]）
—— 第 3 帧 ——
...
—— 第 5 帧 ——
  镇仓入库 10 袋，正大光明记一笔。
  账房：镇仓的账动了！现存 63 袋（added=false，last_changed=99，窗口 (77, 100]）
```
...
2. **第 2 帧**：误报的解剖。小工只是原样抄写，但 `&mut` 解引用即盖章：`last_changed=28` 落在账房的窗口 `(7, 29]` 里——算术成立，账房就喊。顺便注意秒针的步幅：一帧过去，窗口从 7 走到 29，因为 `MinimalPlugins` 这一帧里跑了二十来个系统，每个都让秒针加一。
...
5. **第 5 帧**：正大光明的写法，`99 ∈ (77, 100]`，理所应当被听见。

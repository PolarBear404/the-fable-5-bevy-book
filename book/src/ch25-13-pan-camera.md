...
它不像 FreeCamera 那样配置/状态分家——一件组件全包，字段全是 `Option<KeyCode>`（键位可以单独摘掉，设 `None` 即禁用那一路）。出厂账单：WASD 平移、Q/E 逆顺时针旋转、`+`/`-` 或滚轮缩放、**鼠标左键拖动平移**。这里只把 `pan_speed` 从 500 提到 700（单位是世界单位/秒——2D 里通常就是像素/秒，夜市长街三百多像素一间铺面，700 巡起来不拖沓）。

...
1. **`spawn` 立刻返回 Entity ID。**`.id()` 拿到的 `7v0` 是当场预留的行号——实体还不存在，号码先占住。所以你可以放心把这个 ID 存起来、传给后续命令用，不必等它“真的出生”。
...
看增援的 ID：**`10v0`**——一个全新的行号，而不是史莱姆、骷髅兵空出来的 7 和 8。阵亡者的行号没有立刻转手：`despawn` 把行号送回引擎的回收池，同时给它的**世代号加一**；池里的行号攒够一批才重新发放，何时轮到谁是引擎内务。等行号 8 将来重新上岗，新实体拿到的 ID 会是 `8v1`——这正是上一节埋的悬念：行号会复用，世代号保证复用不会张冠李戴。假如某处还存着骷髅兵的旧 ID `8v0`，拿它去访问 World，引擎一对世代号就知道“此 8 非彼 8”，旧 ID 作废——不会错把 8 号行的新住户当成还魂的骷髅兵。顺带一提，拿过期的 ID 再 `despawn` 一次不会让程序崩溃，Bevy 只是打一条警告，提醒你逻辑里残留着失效引用；但对已销毁实体 `insert` 默认按错误处理、直接 panic——错误处理策略可以定制，第 33 章细讲。

...
<span class="caption">Listing 14-12：完整示例——《长风渡》开机日（src/main.rs）</span>
...
- **`load_embedded_asset!`**。main.rs 在 `src/` 下，于是这个宏接管了 14.8 节手拼 `embedded://` 路径的脏活；场记板从注册到取用，路径字符串一次都没出现。
...
- **运行时保存**：`impl AssetSaver` 是装载器的镜像（async save + `OutputLoader` 指名“谁读得回来”），`save_using_saver` 异步写盘并顺手立 `.meta` 档案；`SavedAsset::from_asset` 打包待存资产，差事丢给 `IoTaskPool`，`Task` 回执用 `check_ready` 每帧问一声——失败分支照样有人管
- **细则**：采样等加载设置走 `load_builder().with_settings(...)`（补丁式）或 `.meta` 档案（全量、跟文件走），同路径只认首次设置；`embedded_asset!` 把素材焊进二进制解决“加载画面自己的素材”；发布期可切 `AssetMode::Processed` 走预加工流水线（第 38 章）
...
3. **全局采样**：Listing 14-10 逐件改采样太啰嗦。查 `ImagePlugin::default_nearest()`，用第 2 章的 `.set()` 手法把全局默认采样换成 Nearest，验证三把剑全部锐利——像素风游戏的标准开局配置。
4. **断粮测试**：把 `assets/` 目录整个改名，分别运行 Listing 14-1 与 Listing 14-11，先预测各自的表现再验证。改回来之后，再用 `BEVY_ASSET_ROOT` 环境变量把资产根指到改名后的目录，让 Listing 14-1 不改代码恢复工作。
5. **批注幸存计划**：Listing 14-9 的定稿里批注全没了——14.7 节解释过这是存档器的本分。现在改变这个本分：给 `Script` 添一个能记住批注（及其行位置）的字段，`ScriptLoader` 解析时收留它们，`ScriptSaver` 存盘时原位写回，验证一稿到定稿的往返中批注一字不丢。

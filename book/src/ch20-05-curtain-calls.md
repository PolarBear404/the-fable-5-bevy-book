...
发球键是这个项目第一个**瞬时**输入——`just_pressed` 只真一帧，而消费它的系统住在 `FixedUpdate`。第 18.6 节开堂审过这桩案子：直接在鼓点里读快照，会丢拍或重拍。药方当时就抓好了：每帧必跑的收集系统把瞬时攒进意图，鼓点上消费、消费即清零。我们的收集站从 20.1 节就立在 `BeforeFixedMainLoop`，现在只是给 `Intent` 添一项：

...
然后——**什么都没有**。阿燕不应（自己不收）、灯笼不应（被挡）、连台口的「落空」都没有（窗口的垫底命中也被挡在名单之外）。全场沉默，正是吸音该有的样子；同一块牌在 3D 里却等于隐身（25.6 的实测）。机制差异在报账环节：sprite 后端把命中**先全数报上去**、让悬停段按 `is_hoverable` 裁决——裁决层的四档是全的；mesh 后端在自家射线阶段就把 `is_hoverable: false` 的剔了，裁决层想管也没得管。跨后端写通用交互时，别假设这两个字段处处同义。

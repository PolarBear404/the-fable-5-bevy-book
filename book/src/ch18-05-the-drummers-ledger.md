...
```rust
{{#include ../../code/ch18-time/examples/listing-18-07.rs:probes}}
```

<span class="caption">Listing 18-7（其一）：两个探针——同一句 `Res<Time>`，两个调度里各报各的（examples/listing-18-07.rs）</span>

```rust
{{#include ../../code/ch18-time/examples/listing-18-07.rs:main}}
```

<span class="caption">Listing 18-7（其二）：鼓点拧到 50 毫秒一拍——`Time::<Fixed>::from_seconds` 是配置入口</span>
...
![示意图：五个并排的水缸代表五帧，水位是攒下的时间，第 1 帧 0、第 2 帧 30、第 3 帧 60 超过 50 的步长线于是跑一拍余 10、第 4 帧 40、第 5 帧 70 再跑一拍余 20](images/ch18/fig-18-06-beats-vs-frames.svg)

<span class="caption">Figure 18-6：鼓师的水缸——每帧倒进 delta，水位够一个步长就跑一拍，零头留缸底</span>
...
![示意图：Time Real 经过 pause、relative_speed、max_delta 三个旋钮汇出 Time Virtual，再经过按步长整拍切割、零头存 overstep 汇出 Time Fixed；通用 Res Time 是一面镜子，在 Update 照出 Virtual，在 FixedUpdate 照出 Fixed](images/ch18/fig-18-07-clock-family.svg)

<span class="caption">Figure 18-7：一族时钟——上游拧旋钮，下游跟着走；通用 `Res<Time>` 只是一面镜子</span>

...
![示意图：左半幅「从灯的视角」，一盏灯向遮挡物与地面射出光线，下方一条深度图色带记下每个方向最近的遮挡距离；右半幅「着色时比对」，同一束光下，地面 A 点的距离正是记录值、受光，被遮挡物挡住的 B 点比记录更远、在影子里](images/ch22/fig-22-03-shadow-map.svg)

<span class="caption">Figure 22-3：阴影贴图的两步——先从灯的视角记下每个方向最近的遮挡距离，着色时再拿当前点的距离与之比对，比记录更远的就是被挡住了</span>

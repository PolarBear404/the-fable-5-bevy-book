...
- **DJ 和记分员两行的先后在漂**——第 2 帧 DJ 在前，其余三帧记分员在前。这不是 bug：两个读者之间没有任何排序约束，而读取只需要共享访问（`Res` + 各自的私有游标），调度器尽可以让它们**并行运行**，打印交错纯属正常。你跑出来的顺序多半和书里不同，多跑几次自己还会变。第 6 章的判断标准在这里复述一遍：它们没有冲突的数据访问，无序是无歧义的。

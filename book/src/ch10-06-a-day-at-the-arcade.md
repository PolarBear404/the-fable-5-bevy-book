...
- **舞台跟着推导态走，而不是跟着源状态**。角色在 `OnEnter(InAction)` 登场、挂 `DespawnOnExit(InAction)` 退场。于是第 4 帧演示局切真人局——源状态换了值——风扇没重启、角色没重生（`ALLOW_SAME_STATE_TRANSITIONS = false` 把同值推导挡掉了）；直到第 10 帧真正回菜单，清场才发生，第 11 帧老板数到 0。要是把这两处都挂在 `GameState::Playing { demo: true }` 上，罗兰接管的瞬间就会看到角色被清掉再生成一遍。**“哪一层状态变了要折腾，哪一层不用”，正是分层建模的全部意义**。
- **计分牌的条件是两层状态的与**：`in_state(Playing { demo: false }).and(in_state(IsPaused::Running))`——真人局且没暂停。第 3 帧（演示）和第 7 帧（暂停）它都沉默，两种沉默来自两个不同的状态机，但写在同一行 `run_if` 里。第 6 章的条件组合在状态机上原样可用。
...
- **状态作用域实体**：spawn 时挂 `DespawnOnExit(s)`/`DespawnOnEnter(s)`，转换期间自动 despawn（含子树）；与“`OnEnter` 搭台”构成标准搭配；同值转换不清场
...
1. **修好重开键**：给 Listing 10-5 的街机加一个真正的“重开本关”——要求重开时连击清零、但角色不闪烁重生。提示：单靠同值转换做不到干净的重开（10-3 节末尾说过它不清场）；试试给 `Playing` 带上一个局数字段 `Playing { round: u32 }`，重开就是 `round + 1`。

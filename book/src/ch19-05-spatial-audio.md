...
场记：阿燕在右舷   58 像素处，左耳 0.00、右耳 0.00（sink 暂停 false）。
```
...
- **衰减**按距离平方反比：`min(1/d², 1)`——一个单位以内不衰减，之后随 d² 跌落；
- **声像**看双耳距离差：近的那只耳朵系数大（范围 0.5 到 1），这就是“左边来的偏左响”；
- 两项相乘，就是这只耳朵实际听到的份额。
...
场记：阿燕在左舷  181 像素处，左耳 0.53、右耳 0.07（sink 暂停 false）。
场记：阿燕在右舷   59 像素处，左耳 0.12、右耳 0.33（sink 暂停 false）。
场记：阿燕在右舷  299 像素处，左耳 0.04、右耳 0.49（sink 暂停 false）。
场记：阿燕在左舷  418 像素处，左耳 0.10、右耳 0.03（sink 暂停 false）。
```

这回有远近也有方位了：阿燕贴近左耳时左 0.53、右 0.07，更声明明白白偏左；走到台口尽头，两耳都只剩零头。耳机里听到的与台账一致——梆子声跟着她从左滑到右，远了轻、近了重。

![两幅截图：左幅阿燕正走到左侧红色耳朵方块上方，HUD 显示声源距台心 200 像素合 2.0 音频单位；右幅阿燕在右侧远端，HUD 显示 390 像素合 3.9 音频单位](images/ch19/fig-19-06-night-patrol.png)

...
窗口照常打开，粗看片场还是那个片场——可盯两眼就露馅了：灯笼柱多出好几根、间距也乱了，地毯宽得不像话。控制台还同步抗议了一声：
...
引擎的意思翻译过来：两台启用的相机往**同一个渲染目标**（主窗口）画画，可它们的 `order` 都是默认的 `0`——谁先画谁后画，没有说法。画面上那些破绽正是这么来的：**排在前面的相机清屏、铺底；排在后面的相机不再清屏，直接把自己的取景叠画上去**。于是你看到的是两幅取景叠印的合成品——两台拍的是同一块场地，地毯同色，拼缝看不出来；灯笼柱和阿燕却各有两套位置，谁露出来、谁被后画那台的地毯盖住，全看叠印次序。本机这一跑是 2 号机叠在上面：画面里只剩它拍的那位阿燕，还偏离了画面正中。**至于谁先谁后，引擎不保证**——换一跑、换台机器都可能翻面。警告只报这一声就归于安静——它混在启动日志里一眼扫过去很容易漏掉，而安静不等于没事。
...
窗口从中线一分为二：左路镜头柔柔地跟着阿燕，右路追着踏雪绕场，两路各自取景，互不相扰。拖动窗口边框，分割线始终咬住中线——`WindowResized` 的功劳。

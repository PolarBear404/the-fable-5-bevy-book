...
糊的原因在第 14 章的库房细则里埋过：图片资产带着**采样设置**，默认是线性过滤——放大时按邻近像素插值，照片这么处理很自然，像素画这么处理就是一团浆糊。当时我们用 `load_with_settings` 逐件改设置救过三把剑；可全游戏的画稿都是像素风时，逐件改太啰嗦。第 14 章练习 3 的答案就是正解，这里转正：
